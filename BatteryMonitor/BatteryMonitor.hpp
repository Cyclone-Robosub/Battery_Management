...
public:
  BatteryMonitor() : Node("BatteryMonitorNode"), startupTime(std::chrono::steady_clock::now()) {
    // SetupADCConnection();
    CurrentChargeSub = this->create_subscription<std_msgs::msg::Float64>(
                "Current_Topic", 10, std::bind(&BatteryMonitor::getCurrent, this, std::placeholders::_1));
    CurrentVoltSub = this->create_subscription<std_msgs::msg::Float64>(
                "Volt_Topic", 10, std::bind(&BatteryMonitor::getVolt, this, std::placeholders::_1));
    ...
    SOCtimer_ = this->create_wall_timer(std::chrono::milliseconds(100), [this]() {
            this->SOC_RealTime_callback();
        });
  }
